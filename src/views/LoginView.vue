...
async function submit() {
  ...
  try {
    ...
    const redirectPath = route.query.redirect || "/projects";
...
<template>
  <div class="flex min-h-screen bg-slate-950 p-4">
    <div
      class="m-auto grid w-full max-w-6xl overflow-hidden rounded-[2rem] bg-white shadow-soft lg:grid-cols-[1.05fr_0.95fr]"
    >
      <section
        class="hidden bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 p-10 text-white lg:block"
      >
        <div class="flex h-full flex-col justify-between">
          <div>
            <div
              class="mb-8 flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 backdrop-blur"
            >
              <KanbanSquare class="h-7 w-7" />
            </div>
            <h1
              class="max-w-lg text-5xl font-black leading-tight tracking-tight"
            >
              Plan, track and ship work faster.
            </h1>
            <p class="mt-5 max-w-md text-base leading-7 text-slate-300">
              Jira clone mini dùng Vue, Tailwind và WebSocket realtime. Phù hợp
              để học cách nối frontend với FastAPI.
            </p>
          </div>
      ...
      <section
        class="flex min-h-[680px] items-center justify-center p-6 sm:p-10"
      >
        <div class="w-full max-w-md">
          ...
          <p class="mt-2 text-sm text-slate-500">
            Dùng tài khoản đã đăng ký ở backend FastAPI.
          </p>
          ...
          <p class="mt-6 text-center text-sm text-slate-500">
            No account?
            <RouterLink
              class="font-bold text-slate-950 hover:underline"
              to="/register"
