...
const emit = defineEmits(["create-issue", "open-issue", "drag-issue", "drop-issue"]);

const isOver = ref(false);
const sortedIssues = computed(() => [...props.issues].sort((a, b) => a.position - b.position));
...
<template>
  ...
  >
    <div class="mb-3 flex items-center justify-between px-2 py-1">
      <div>
        <h2 class="text-sm font-black uppercase tracking-wide text-slate-700">{{ column.name }}</h2>
        <p class="text-xs font-medium text-slate-400">{{ sortedIssues.length }} issues</p>
      </div>
      <button class="rounded-xl bg-white p-2 text-slate-500 shadow-sm transition hover:text-slate-950" @click="emit('create-issue', column)">
        <Plus class="h-4 w-4" />
      </button>
    </div>

    <div :class="['flex flex-1 flex-col gap-3 rounded-2xl p-1 transition', isOver ? 'bg-blue-100/70 ring-2 ring-blue-200' : '']">
      <IssueCard
